...
export const leadsRouter = asyncHandler(async (req, res, url) => {
  ...
  if (path === '/leads' && method === 'POST') {
    ...
    const { lead_id, contact_id, is_new } = await createLead({ ...body, source: body.source || 'manual' });
    if (body.stage_id) await moveLead(lead_id, body.stage_id, user.id);
    if (body.assigned_user_id) await LeadRepository.updateFields(lead_id, { assigned_user_id: body.assigned_user_id });
  ...
  if (idMatch && method === 'GET') {
    const lead = await getLeadFull(idMatch[1]);
    if (!lead) throw ApiError.notFound('Lead');
    const notes = await LeadNoteRepository.listByLead(idMatch[1]);
    const events = await LeadEventRepository.listByLead(idMatch[1]);
  ...
  if (stageMatch && method === 'POST') {
    ...
    const ok = await moveLead(stageMatch[1], body.stage_id, user.id);
  ...
  if (noteMatch && method === 'POST') {
    ...
    const id = await LeadNoteRepository.insert({ lead_id: noteMatch[1], user_id: user.id, content: body.content });
    await recordEvent({ lead_id: noteMatch[1], user_id: user.id, type: 'note_added', description: 'Nota adicionada' });
